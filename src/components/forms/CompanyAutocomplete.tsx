...
interface Props {
    value: string;
    onChange: (companyName: string, clientId?: string, clientData?: any) => void;
    division?: string;
    placeholder?: string;
    className?: string;
    disabled?: boolean;
...
export default function CompanyAutocomplete({ 
    ...
    const handleSelect = (client: Client) => {
        const chosenCompany = client.companyName?.trim() || client.name?.trim() || "";
        setInputValue(chosenCompany);
        onChange(chosenCompany, client.id, client);
        setShowSuggestions(false);
    };
                    ...
                    {suggestions.length > 0 ? (
                        <>
                            {suggestions.map((client) => {
                                const compName = client.companyName?.trim() || client.name?.trim() || "Independent Client";
                                const contactName = client.contactPerson && client.contactPerson !== "N/A" ? client.contactPerson : client.name;
                                return (
                                    <button
                                        key={client.id}
                                        type="button"
                        ...
                        <div className="p-6 text-center flex flex-col items-center">
                            ...
                            <p className="text-xs text-slate-400 mb-4 text-center px-4">
                                Please select from the dropdown suggestions or search across all sectors.
                            </p>
                            {!ignoreDivision && (
                                <button
                                    type="button"
